...
from ltl_tools.product import ProdAut, ProdAut_Run
from ltl_tools.ts import distance, reach_waypoint
from ltl_tools.discrete_plan import dijkstra_plan_networkX, dijkstra_plan_optimal, improve_plan_given_history, has_path_to_accept, dijkstra_path_networkX, opt_path_in_prefix, opt_path_in_suffix, opt_path_jit, add_temp_task

import time


class ltl_planner(object):
        def __init__(self, ts, hard_spec, soft_spec, beta, robot_name = 'turtlebot'):
        ...
        # ------------------------------
        # WORKSPACE UPDATE
        def update_workspace(self, pos1, pos2):
                sense_info = dict()
                sense_info['edge'] = ({(pos1, pos2),(pos2, pos1)},set())
                self.product.graph['ts'].graph['region'].update_after_edges_change(sense_info)
                self.product.graph['ts'].update_after_edges_change(sense_info)
                self.product.update_after_edges_change(sense_info)
        ...
        def add_temp_task(self, temp_task):
                if self.robot_name == 'turtlebot':
                        reg_s = ((temp_task[0],temp_task[1], 0.00), 'None')
                        reg_g = ((temp_task[2],temp_task[3], 0.00),'None')
                elif self.robot_name == '2D_agent':
                        reg_s = ((temp_task[0],temp_task[1]), 'None')
                        reg_g = ((temp_task[2],temp_task[3]),'None')
                else:
                        print('Robot model not recognized')
                        return
                t_sg = temp_task[4]
                best_line, best_precost = add_temp_task(self.product, self.run, self.index, self.segment, reg_s, reg_g, t_sg)
                print(best_line)
                self.run.update_line(self.product, best_line, best_precost)                
